...
function getMealCandidates(mealType, state, metrics, mode = "default") {
...
function buildSlotOptions(config, excludeSignatures, state, mode = "default") {
  ...
  const candidates = getMealCandidates(config.key, state, metrics, mode);
  const options = [];
...
export function generateMealOption(config, seed, excludeSignatures, state, mode = "default") {
  const options = buildSlotOptions(config, excludeSignatures, state, mode);
...
export function generateDietPlan(state) {
  ...
  MEAL_CONFIGS.forEach((config) => {
    ...
    const meal = generateMealOption(config, seed, new Set(), state, "generation");
...
export function getAlternativeItemsForState(meal, currentItem, state) {
  const scaled = getMealCandidates(meal.mealType, state, getMetrics(state), "swap");
...
export function getAlternativeMeals(meal, mealIndex, state) {
  ...
  const allOptions = buildSlotOptions(config, signatures, state, "swap");
